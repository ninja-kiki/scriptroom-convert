...
function assembleNameGlossary(profile) {
  ...
  return `\n\n[인물 이름 한글 표기 — 이 작품의 표준 표기(자막이 있으면 자막 기준). 같은 인물을 다른 음역(애니/아니/Ani 식)으로 적지 말 것.
★적용 범위: @인물 큐 줄 · 대사 속 호명 · 지문 속 인물명 — 세 곳 모두 이 표기로 통일한다(@MARTY → @마티).
★사전에 없는 단역 큐(BARTENDER · COP 1 · BYSTANDER 등 역할명)도 한국어로 옮기되(바텐더 · 경찰 1 · 행인), 작품 내내 같은 역할은 같은 표기를 쓸 것.
★V.O./O.S./CONT'D 등 수식어는 영문 약자 그대로 둔다(@마티 (V.O.)).]\n${pairs.join(' · ')}`
}
...
반드시 **JSON만** 출력 (설명·코드펜스 금지). 형식:
{
  ...
  "nameMap": { "ANI": "애니", "IVAN": "이반" },   // 인물 이름의 영문(대사 큐 표기) → 한글 표기. ★자막이 있으면 임의로 음역하지 말고 자막에 실제로 쓰인 인물 이름 표기를 그대로 채집해 넣을 것(자막이 표준). 자막에 안 나오는 인물만 보완. 한국어 자막이 없으면 외래어 표기법·관용 표기(통용되는 작품이면 그 공식 표기)에 맞춰 한 번만 정해 전 인물을 채워라 — 핵심은 작품 내내 한 인물=한 표기로 고정되는 것. 주요 인물 전원 + 두 번 이상 등장하는 단역·역할명 큐(BARTENDER→바텐더, COP 1→경찰 1, BYSTANDER→행인 식)까지 최대 40개. 역할명은 음역하지 말고 한국어 역할어로 옮긴다. 애칭·풀네임이 다르면 둘 다 키로.
