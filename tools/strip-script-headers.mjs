...
const PATS = [
  ...
  // 개정색 Rev. + (월 일 '연도) + 시각 + 페이지: "Tan Rev. (Jan 31 '18) - 1518 3bA."
  ...
  // TV 각본 머리글(제목 + #화수 + "부제" + 판본 + 날짜 + 페이지):
  //   'BETTER CALL SAUL #613 "Saul Gone" FINAL 1/19/22 9.'
  /[A-Z][A-Z' ]{2,40}\s*#\d+[A-Za-z]?\s*"[^"]{1,60}"\s*[A-Z]+\s*\d{1,2}\/\d{1,2}\/\d{2,4}\s*[\d]{0,4}[A-Za-z]?\.?/gi,
