...
export const DEFAULT_TRANSLATE_GUIDELINES = `구조·마커 유지(@·전환지시어·대사 앞 '- ' 형태 그대로). 직역 금지, 장면 맥락 번역. 욕설 질감 살린 자연스러운 구어체.

규칙:
- 씬 헤딩 번역: # INT.→내부. EXT.→외부. INSERT/INTERCUT/MONTAGE/SERIES OF SHOTS는 번역하지 말고 인서트/인터컷/몽타주/시리즈 오브 샷처럼 업계에서 쓰는 음차 그대로, 장소·시간대도 한국어
- ★@인물명은 한국어 음차로 번역(@MARTY→@마티, @JOHANSSEN→@조핸슨). V.O./O.S./CONT'D 같은 수식어는 영어 약자 그대로 유지(@JOHANSSEN (V.O.)→@조핸슨 (V.O.)). O.S./V.O.가 단독으로 인물칸에 오면(화자 미상 무전·방송 등) 번역하지 말고 그대로 둔다. 같은 인물은 작품 전체에서 동일한 한국어 표기로 통일
- ★대사 줄 맨 앞의 '- ' 는 대사를 표시하는 구조 마커다(#·@처럼). 지우거나 번역하지 말고 그대로 유지
- 지문 속 대문자 인물명 한국어로(BLAKE→블레이크, SOLDIERS→병사들) — @인물명과 동일 인물이면 표기를 통일할 것
- (괄호지문) 번역.
- ★화면 삽입 텍스트(자막/타이틀/크레딧)는 반드시 자기만의 독립된 한 줄(앞뒤 빈 줄)로 — 절대 문장 중간 인라인 금지:
    · 위치·시간·상황 자막(SUPER:/LEGEND:/CHYRON:/SUBTITLE:/TITLE: 뒤 장소·연도·날짜, 또는 "a CHYRON/TITLE reads 'X'") → [자막: 내용]
    · 제작진 표기(Screenplay by/Directed by/Story by/Based on 등) → [크레딧: 내용]
    · 영화 제목 카드 → [타이틀: 내용] (단 원제 자체는 영어 유지)
    · 기존 [CREDIT:]→[크레딧:], [SUPER:]→[자막:]. 내용은 번역
    · 주의: "SUPER TIGHT" "SUPER-CLOSE" 등 콜론 없는 카메라 지시는 자막 아님 — 지문 그대로 둘 것
    · ★원문에 이미 "자막:"/"타이틀:"/"크레딧:" 같은 라벨이 텍스트로 박혀 있으면(예: "자막: Sol 24") 그 라벨을 지우고 내용만 뽑아 [자막: Sol 24]처럼 한 번만 감싼다. [자막: 자막: X]처럼 이중으로 감싸지 말 것
