...
import { estTokens, buildDialogueSample, stripMeta, normalizeSpacing, cleanOutput, buildBatches, splitByHeading, splitScenes } from './lib/pipeline.js'
...
export default function App() {
  ...
  async function runWithConcurrency(items, fn, concurrency) {
    ...
    const workers = Array(Math.min(concurrency, queue.length)).fill(null).map(async () => {
      while (queue.length > 0) {
        ...
        if (item === undefined) return   // 일시정지 사이에 다른 워커가 큐를 비웠을 때 fn(undefined) 방지
        await fn(item)
      }
    })
  ...
  async function handleLoad({ scriptFile, smiFile, title: t }) {
    ...
    if (ext === 'txt' && detectFileType(rawText) === 'formatted') {
      const rawScenes = splitScenes(rawText).map((raw, i) => ({
        id: i, raw,
